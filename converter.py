...
try:
    ...
    print 'Failed importing fbx sdk modules'
    sys.exit(1)
...
def quantize(pList, pStride, pMin, pMax):
    ...
    # TODO
    if pStride == 1:
        lDecodeMatrix = [lDivider[0], 0,
            pMin[0], 1]
    elif pStride == 2:
        lDecodeMatrix = [lDivider[0], 0, 0,
            0, lDivider[1], 0,
            pMin[0], pMin[1], 1]
    elif pStride == 3:
        lDecodeMatrix = [lDivider[0], 0, 0, 0,
            0, lDivider[1], 0, 0,
            0, 0, lDivider[2], 0,
            pMin[0], pMin[1], pMin[2], 1]
    elif pStride == 4:
        lDecodeMatrix = [lDivider[0], 0, 0, 0, 0,
            0, lDivider[1], 0, 0, 0,
            0, 0, lDivider[2], 0, 0,
            0, 0, 0, lDivider[3], 0,
...
def ProcessUV(uv, scaleU, scaleV, translationU, translationV):
    for i in range(len(uv)):
        uv[i] = [uv[i][0] * scaleU + translationU,
...
def ConvertMesh(pScene, pMesh, pNode, pSkin, pClusters):
    ...
    if lAllSameMaterial:
        ...
        lPrimitivesList.append(CreatePrimitiveRaw(lTmpIndex, False,
            ...
            if lMaterialLayer.GetMappingMode() == FbxLayerElement.eByPolygon:
                for k in range(len(lMaterialIndices)):
                    if lIndexArray.GetAt(k) >= 0:
                        # index in top material layer will overwrite the bottom
                        # material layer
        ...
        for lIdx in lMaterialIndices:
            if not lIdx in lMaterialsPrimitivesMap:
                ...
                lPrimitivesList.append(CreatePrimitiveRaw(lGLTFMaterialIdx, lIsMaterialInSecondLayer[lIdx],
                    lScaleU, lScaleV, lTranslationU, lTranslationV))
    ...
    for i in range(len(lPrimitivesList)):
        ...
        if len(lPrimitive['texcoords0']) > 0:
            ProcessUV(lPrimitive['texcoords0'],
        ...
        if len(lPrimitive['texcoords1']) > 0:
            ProcessUV(lPrimitive['texcoords1'],
                lPrimitive['scaleU'], lPrimitive['scaleV'],
                lPrimitive['translationU'], lPrimitive['translationV'])
...
def ConvertSceneNode(pScene, pNode, pPoseTime):
    ...
    if pNode.GetVisibility() and lMesh:
        ...
        # (Mesh splitted by material may have multiple MeshAttribute in one
        # node)
...
def ConvertNodeAnimation(pGLTFAnimation, pAnimLayer, pNode, pSampleRate, pStartTime, pDuration):
    ...
    curves = [pNode.LclTranslation.GetCurve(pAnimLayer, 'X'),
    ...
    if lDuration > 0:
        ...
        lTimeChannel, lTranslationChannel, lRotationChannel, lScaleChannel = FitLinearInterpolation(lTimeChannel, lTranslationChannel, lRotationChannel, lScaleChannel)
...
# Each node can have two pivot context.  The node's animation data can be
# converted from one pivot context to the other
...
def PrepareBakeTransform(pNode):
    ...
    # pNode.SetUseQuaternionForInterpolation(FbxNode.eDestinationPivot,
    # pNode.GetUseQuaternionForInterpolation(FbxNode.eSourcePivot));
...
def CorrectImagesPaths(pFilePath):
    ...
    for lGLTFImage in lib_images:
        ...
        # FBX SDK extracts zip input files to temp folder, so use lGLTFImage
        # uri instead to find temp folder
        ...
        if lUri:
            ...
            head , tail = os.path.split(lUri)
            textureUri = 'textures/' + tail
            textureUri = textureUri.replace(r'[\\\/]+', os.path.sep)
            lGLTFImage['uri'] = textureUri
...
def Convert(filePath,
    ...
    if binary:
        ...
        lOutFile.close()
        print "GLTF file generated successfully"


if __name__ == '__main__':

    parser = argparse.ArgumentParser(description='Rplat glTF converter', add_help=True)
    parser.add_argument('-i', '--input', type=str, help='Input file to be converted')
    ...
    if not args.output:
        ...
        lBasename, lExt = os.path.splitext(args.input)
        if args.binary:
    ...
    mFilePath = args.input

    mFileDir = Path(mFilePath).parents[0]
    manager = fbx.FbxManager.Create()
    importer = fbx.FbxImporter.Create(manager, 'mImporter')
    status = importer.Initialize(mFilePath)

    if not status:
        print "Input file is not acceptable" 
        print 'Error %s' % importer.GetLastErrorString()
        sys.exit()

    mFileExtension = mFilePath.rsplit('.', 1)[1].lower()
    mFileName = Path(mFilePath).stem
    if lOutputDirSpecified:
        mExportDir = os.path.join(os.getcwd(),args.output)
    else:
        mExportDir = os.path.join(os.getcwd(),slugify(unicode(mFileName) + "_export"))

    print 'exporting to {}'.format(mExportDir)
    
    if not os.path.exists(mExportDir):
        os.makedirs(mExportDir)

    # standardized fbx file will be copied here
    mFbxDir = os.path.join(mExportDir,'fbx')
    if not os.path.exists(mFbxDir):
        os.makedirs(mFbxDir)    

    mGltfDir = os.path.join(mExportDir,'gltf')
    if not os.path.exists(mGltfDir):
        os.makedirs(mGltfDir)   
        
    scene = fbx.FbxScene.Create(manager, 'mScene')
    ...
    for i in range(0, textureArray.GetCount()):
        ...
        if texture.ClassId == fbx.FbxFileTexture.ClassId:
            mTexturePath = texture.GetFileName()
            if(mTexturePath == ""):
                continue
            mTextureFormat = mTexturePath.rsplit('.', 1)[1].lower()
            mTextureName = Path(mTexturePath).stem
            
            if "\\" in mTextureName:
                mTextureName = mTextureName.rsplit('\\',1)[1]
            mTextureName = mTextureName.replace("VRayCompleteMap","_baseColor")
            mTextureName = mTextureName.replace(r'[\\\/]+', os.path.sep)

            mTextureFullPath = os.path.join(mFbxTexturesDir,mTextureName + "." + mTextureFormat)
            if not os.path.isfile(mTexturePath):
                print 'MISSING TEXTURE: %s [Will be replaced by a dummy image]' % mTexturePath
                #TODO: replace textures by according their names

                img = Image.new('RGB', (512, 512), color = (255, 105, 180))
                d = ImageDraw.Draw(img)
                fnt = ImageFont.truetype('fonts/OpenSansRegular.ttf', 15)
                d.text((10,250), "Missing Texture" + mTextureName, font=fnt, fill=(255,255,255))
                img.save(mTextureFullPath)

            else:            
                print 'Texture %s: %s' % (i + 1, mTextureName + "." + mTextureFormat)
                im = Image.open(mTexturePath)            
                if mTextureFormat != 'jpg' and mTextureFormat != 'jpeg' and mTextureFormat != 'png':
                    print 'Converting %s image to png format' % mTextureFormat
                    mTextureFullPath = os.path.join(mFbxTexturesDir,mTextureName + ".png")    
    ...
    mExportedFbx = os.path.join(mFbxDir, 'scene.fbx')
    exporter.Initialize(mExportedFbx, 1)
    exportStatus = exporter.Export(scene)

    if not exportStatus:
        print "FBX file can not be saved!" 
        #print 'Error %s' % exporter.GetLastErrorString()
        sys.exit()
